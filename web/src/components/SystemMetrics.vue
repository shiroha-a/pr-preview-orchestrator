    ...
    <div class="space-y-3 p-4">
      ...
      <div>
        <div class="mb-1 flex justify-between text-xs text-gray-500">
          <span>メモリ</span>
          <span
            >{{ gb(metrics.memory.used) }} / {{ gb(metrics.memory.total) }} GB ({{ memPct }}%)</span
          >
        </div>
      ...
      <div>
        <div class="mb-1 flex justify-between text-xs text-gray-500">
          <span>ディスク (/)</span>
          <span
            >{{ gb(metrics.disk.used) }} / {{ gb(metrics.disk.total) }} GB ({{ diskPct }}%)</span
          >
        </div>
        <div class="h-2 overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700">
          <div
        ...
        <div class="space-y-1">
          ...
          >
            ...
            <span class="shrink-0 text-gray-500"
              >CPU {{ cont.cpu }} ・ MEM {{ cont.memUsage }}</span
            >
          </div>
        </div>
      </div>
