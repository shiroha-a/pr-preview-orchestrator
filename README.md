...
| `BUILD_MODE_DEFAULT`               | ビルドの既定モード(`auto` / `remote` / `local`) | `auto`             |
| `SERVER_MODE`                      | プロセスの役割(`orchestrator` / `agent`)        | `orchestrator`     |
| `ORCHESTRATOR_URL` / `AGENT_TOKEN` | agentモード時の接続先とトークン                 | (なし)             |
...
Dockerイメージの**ビルドだけ**を別ホストへ委譲できます。プレビューの実行・公開・監視は
従来どおりオーケストレーター側で行うため、ポート・トンネル・volume等の挙動は変わりません。

仕組み(pull型):

1. ビルドサーバー上のエージェントがオーケストレーターのAPIをlong-pollしてジョブを取得
   (通信は常にエージェント→本体のアウトバウンドのみ。受信ポート開放は不要)
2. エージェントが対象リポジトリを自分でclone(本体が解決したcommit SHAを再現)し、
   オーバーレイ/書き換えを適用して `docker compose build`
3. ビルドしたイメージを `docker save | gzip` でストリーム転送 → 本体が `docker load`
4. 本体がローカルで `docker compose up -d`(再ビルドなし)

セットアップ:

1. WebUIの**設定**ページ →「外部ビルドサーバー」→ 名前を入力して**登録**
2. 表示された起動コマンド(ワンタイム表示のトークン入り)をビルドサーバーで実行:

```bash
# イメージは本リポジトリのDockerfileでビルドしておく
docker build -t pr-preview-orchestrator-agent .

docker run -d --name pr-preview-agent \
  --restart unless-stopped \
  -e SERVER_MODE=agent \
  -e ORCHESTRATOR_URL=<オーケストレーターのURL> \
  -e AGENT_TOKEN=<登録時に発行されたトークン> \
  -v /var/run/docker.sock:/var/run/docker.sock \
  -v pr-preview-agent-data:/data \
  pr-preview-orchestrator-agent
```

3. 設定ページの一覧で**オンライン**になれば接続完了

ビルドモードはグローバル既定(`BUILD_MODE_DEFAULT`)に加え、リポジトリ/プロファイル単位で選択できます:

- `auto`(既定): オンラインのビルドサーバーがあれば委譲し、不在・失敗時はローカルビルドへフォールバック
- `remote`: 常に外部ビルドサーバーを使う(不在時はビルド失敗)
- `local`: 常にローカルでビルド(従来動作)

補足:

- ビルドキャッシュはビルドサーバー側に蓄積されるため、本体のディスク圧迫(#67/#69)が軽減されます。
- private リポジトリのcloneに必要なトークンはジョブごとに注入されます。clone/fetch後は
  workspaceの`.git/config`(remote URL)からも除去され、ジョブ実行中を除きエージェントに残りません。
- イメージ転送はGB級になることがあります。LAN内での利用を推奨します。
- エージェントのトークンやジョブ内のGitHubトークンは`ORCHESTRATOR_URL`が`http://`だと平文で流れます。
  **LAN外を経由する場合はHTTPS(リバースプロキシ等)を必須**としてください。
...
- **ファイル書き換えルール**: clone後・起動前に、対象リポジトリ内の既存ファイルを正規表現で書き換えます。
  ...
  - PR情報の変数も使えます(issue #75): `{{PR_NUMBER}}`(PR番号)/ `{{PR_TITLE}}`(PRタイトル)/
    ...
    `{{PR_NUMBER}}` と `{{PR_TITLE}}` はどちらもブランチ名になります。値はエスケープされないため、
    YAML等に埋め込む際の引用は設定側で行ってください。
...
設定画面の「通知(ビルド完了)」で、ビルドの完了(成功/失敗)を受け取れます。

- **通知音**: プレビューのページを開いているとき、ビルド完了時にチャイムを鳴らします
  (成功と失敗で音が異なります)。設定はブラウザごとに保存されます。
- **プッシュ通知**: タブを閉じていてもブラウザ通知で受け取れます。通知クリックで
  該当のPR/ブランチページが開きます。必要な鍵(VAPID)はサーバーが初回に自動生成して
  DBへ保存するため、環境変数などの設定は不要です。
  - プッシュ通知は**HTTPSまたはlocalhostでのみ**利用できます(Service Workerの制約)。
    平文HTTPでLANアクセスしている場合は通知音のみ利用できます。
