...
- **`.env` の値に `$` を含む場合はシングルクォートで囲んでください**
  (例: `ADMIN_PASSWORD='pa$w0rd'`)。composeは `.env` の値をさらに変数として展開するため、
  クォート無しやダブルクォートだと `$` 以降が失われます(`pa$w0rd` → `pa`)。
  パスワードが壊れたまま起動すると、正しいパスワードを入力しても401が返り、
  ブラウザの認証ダイアログが出続けます。認証に失敗すると
  `[auth] Rejected Basic auth for user "..."` がログに出るので、
  `docker compose logs` で確認できます。
